...
function getVerificationMethod(did, index, algo, encoding, value, controller, chainId) {
  ...
  switch (encoding) {
    ...
    case 'hex':
      ...
      break
    case 'blockchain':
      verificationMethod.blockchainAccountId = `eip155:${chainId}:${value}`
      break
    case 'base64':
      verificationMethod.publicKeyBase64 = Buffer.from(value.slice(2), 'hex').toString('base64')
      break
    case 'base58':
      verificationMethod.publicKeyBase58 = bs58.encode(Buffer.from(value.slice(2), 'hex'))
      break
    case 'pem':
      verificationMethod.publicKeyPem = Buffer.from(value.slice(2), 'hex').toString()
      break
  }
  return verificationMethod
}

function wrapDidDocument(did, id, controller, history, chainId, mode) {
  ...
  return {
    didDocument: new DIDDocument(doc, mode),
  }
}

export function getResolver(config = {}) {
  const iface = new ethers.Interface(DID_REGISTRY_CONTRACT_GAS.abi)
  ...
  async function changeLog(identity, registry) {
    ...
    while (previousChange) {
      ...
      for (const log of logs) {
        ...
        if (Number(event.args.previousChange) < blockNumber) {
          previousChange = event.args.previousChange
        }
      }
    }

    return { controller, history, chainId }
  }
  ...
  async function resolve(did) {
    ...
    const registry = new ethers.Contract(network.registry, DID_REGISTRY_CONTRACT_GAS.abi, provider)

    const { controller, history, chainId } = await changeLog(id, registry)
