...
export function getExistingMethods(verificationMethods, relationships) {
  return relationships.filter((relationship) => verificationMethods.find((vm) => vm.id === relationship))
}
...
export function interpretIdentifier(identifier) {
  let id = identifier
  if (!id.startsWith('did:lac')) throw new Error('Invalid identifier format, must start with "did:lac"')

  const [network, address] = id.replace('did:lac:', '').split(':')
