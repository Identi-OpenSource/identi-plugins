...
export class SDJwtPlugin implements IAgentPlugin {
  ...
  async createVerifiableCredentialSDJwt(
    ...
    return { credential, credentialDecode: await sdjwt.decode(credential) }
  }
  ...
  async createVerifiablePresentationSDJwt(
    ...
    const cred = await SDJwt.fromEncode(args.credential, this.algorithms.hasher)
    if (!cred?.jwt?.payload?.credentialSubject?.hasOwnProperty('id')) {
      throw new Error('invalid_argument: credentialSubject.id must not be empty')
    }
    const holder: any = cred?.jwt?.payload?.credentialSubject
    const identifier = await context.agent.didManagerGet({ did: holder.id })
    const { key, alg } = SDJwtPlugin.getSigningKey(identifier)
    ...
    const credential = await sdjwt.present(args.credential, args.presentationFrame)

    return {
      presentation: credential,
      presentationDecode: await sdjwt.decode(credential),
    }
  }
   ...
   * @returns
   */
  async verify(sdjwt: any, context: IRequiredContext, data: string, signature: string) {
